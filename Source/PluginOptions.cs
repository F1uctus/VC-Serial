...
namespace Serial {
    ...
    public static class PluginOptions {
        ...
        public static bool SelectedPortLogging       = true;
        public static bool SelectedPortLogAutoScroll = true;
            ...
            foreach (XmlNode option in optionsXml.DocumentElement.SelectNodes("Option")) {
                ...
                switch (option.Attributes["name"].Value) {
                    case nameof(GenEventOnReceive):
                        ...
                        break;
                    case nameof(SelectedPortLogging):
                        bool.TryParse(optionValue, out SelectedPortLogging);
                        break;
                    case nameof(SelectedPortLogAutoScroll):
                        bool.TryParse(optionValue, out SelectedPortLogAutoScroll);
                        break;
                }
            }
        }
        ...
        public static void SaveOptionsToXml() {
            using (var writer = new XmlTextWriter(optionsPath, new UTF8Encoding()) {
                ...
                writer.WriteStartElement("Options");
                {
                    ...
                    WriteOption(writer, nameof(SelectedPortLogging),       SelectedPortLogging.ToString());
                    WriteOption(writer, nameof(SelectedPortLogAutoScroll), SelectedPortLogAutoScroll.ToString());
                }
